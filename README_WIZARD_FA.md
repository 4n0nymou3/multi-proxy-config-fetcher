...
فایل `src/user_settings.py` را ویرایش کنید تا رفتار fetcher را کنترل کنید:
...
فایل `xray_fragment_loadbalanced_config.json` یک مکانیزم پیشرفته و دومرحله‌ای قطعه‌بندی TLS ClientHello را روی تمام کانفیگ‌ها اعمال می‌کند که می‌تواند در برابر فیلترینگ مبتنی بر DPI کمک‌کننده باشد. تمام پارامترهای آن در فایل `src/fragment_settings.py` قرار دارند:
...
بستگی به `USE_MAXIMUM_POWER` در `src/user_settings.py` دارد. با مقدار `True`، حداکثر تعداد ممکن از منابع پیکربندی‌شده‌ی شما دریافت می‌شود.

**س: می‌توانم منابع خودم را اضافه کنم؟**
بله — آن‌ها را به `SOURCE_URLS` در `src/user_settings.py` اضافه کنید (بخش [سفارشی‌سازی منابع و تنظیمات](#-سفارشی‌سازی-منابع-و-تنظیمات) را ببینید).
