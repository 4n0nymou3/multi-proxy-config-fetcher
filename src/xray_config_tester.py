            ...
            with managed_process([self.xray_path, 'run', '-c', config_file]) as process:
                ...
                if not wait_for_port(process, probe_port, max_wait=5.0):
                    ...
                    diagnostic_lines = [
                        ...
                        and 'anti-censorship' not in line
                    ]
                    fatal_lines = [line for line in diagnostic_lines if 'Failed to start' in line or '[Fatal]' in line]
                    if fatal_lines:
                    ...
                    if len(prepared) == 1:
                        logger.warning(f"Isolated failing config: {prepared[0][3][:300]}")
